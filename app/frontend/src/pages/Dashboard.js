		...
		<div className={styles.content}>
			...
			<div className={styles.coreContent}>
				...
				<main className={styles.main}>
					<Section title="Dashboard" children={categories.map((asset) => (
						<AssetCard key={asset.id} name={asset.name} money={asset.money}/>
					))} />
					
					<Section title="Funcionalidades" children={functionalities.map((functionality) => (
						<FunctionalityCard path={functionality.path} key={functionality.id} icon={functionality.icon} alt={functionality.name} name={functionality.name} />
					))} />
				</main>
			</div>
