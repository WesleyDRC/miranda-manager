...
let CreateCategoryUseCase = class CreateCategoryUseCase {
    ...
    execute(_a) {
        return __awaiter(this, arguments, void 0, function* ({ name }) {
            const categoryFound = yield this.categoryRepository.findByName(name);
            if (categoryFound) {
                throw new AppError_1.AppError(categoryContants_1.categoryConstants.ALREADY_EXISTS, 409);
            }
            const createCategory = yield this.categoryRepository.create({ name });
