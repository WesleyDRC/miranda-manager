...
exports.GetRentDashboardUseCase = void 0;
...
let GetRentDashboardUseCase = class GetRentDashboardUseCase {
    constructor(rentRepository) {
        this.rentRepository = rentRepository;
    }
    execute(_a) {
        return __awaiter(this, arguments, void 0, function* ({ userId }) {
            const rents = yield this.rentRepository.findAll(userId);
            // Auto-fill missing months for all rents to ensure stats are accurate
            const rentsWithMonths = yield Promise.all(rents.map((rent) => __awaiter(this, void 0, void 0, function* () {
                const months = yield this.fillMissingMonths(rent);
                return Object.assign(Object.assign({}, rent), { months });
            })));
            ...
            rentsWithMonths.forEach((rent) => {
                if (rent.months) {
                    rent.months.forEach((month) => {
                        if (month.id)
                            allMonthIds.push(month.id);
                    });
                }
            });
            // Fetch payments, expenses, receipts in bulk
            const payments = yield this.rentRepository.findRentPaymentsByMonthIds(allMonthIds);
            const expenses = yield this.rentRepository.findRentExpensesByMonthIds(allMonthIds);
            const receipts = yield this.rentRepository.findRentReceiptsByMonthIds(allMonthIds);
            ...
            const detailedRentals = rentsWithMonths.map((rent) => {
                ...
                const mappedMonths = (rent.months || []).map((month) => {
                    ...
                    const isCurrentMonthObj = monthDate.getMonth() === currentMonthIndex &&
                    ...
                    if (!isPaid) {
                        ...
                        if (!nextDue) {
                            nextDue = (0, formatDateToDDMMYY_1.formatDateToDDMMYY)(monthDate);
                        }
                    }
                    return {
                        id: month.id,
                        dateMonth: (0, formatDateToDDMMYY_1.formatDateToDDMMYY)(monthDate),
                ...
                return {
                    ...
                    status,
                    lastPaymentDate: lastPaymentDate ? (0, formatDateToDDMMYY_1.formatDateToDDMMYY)(lastPaymentDate) : "—",
                    nextDue: nextDue || "—",
                    months: mappedMonths,
                };
            });
    ...
    fillMissingMonths(rent) {
        return __awaiter(this, void 0, void 0, function* () {
            const rentMonths = yield this.rentRepository.findAllRentMonthByRentId(rent.id);
            const [day, month, year] = rent.startRental.split("/").map(Number);
            const sortedMonths = [...rentMonths].sort((a, b) => new Date(a.dateMonth).getTime() - new Date(b.dateMonth).getTime());
            ...
            if (sortedMonths.length > 0) {
                ...
                const firstMonth = yield this.rentRepository.createRentMonth({
            ...
            for (let date = new Date(nextDate); date <= endDate; date.setMonth(date.getMonth() + 1)) {
                ...
                const exists = sortedMonths.some((m) => new Date(m.dateMonth).getMonth() === date.getMonth() &&
                    new Date(m.dateMonth).getFullYear() === date.getFullYear());
                if (!exists) {
                    const created = yield this.rentRepository.createRentMonth({
...
exports.GetRentDashboardUseCase = GetRentDashboardUseCase;
exports.GetRentDashboardUseCase = GetRentDashboardUseCase = __decorate([
    (0, tsyringe_1.injectable)(),
    __param(0, (0, tsyringe_1.inject)("RentRepository")),
    __metadata("design:paramtypes", [Object])
], GetRentDashboardUseCase);
