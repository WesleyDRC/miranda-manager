...
export class CreateFinanceUseCase implements IUseCase {
	...
	async execute({name, rent, categoryId, userId}: IStoreFinanceDTO): Promise<IFinance> {

		await this.ensureFinanceDoesNotExist(name)

		const category = await this.getCategory(categoryId)

		const rentId = await this.createRentIfNeeded(
			category.name,
			rent,
			userId
		)
	...
	private async createRentIfNeeded(categoryName: string, rent: any, userId: string): Promise<string | null> {
